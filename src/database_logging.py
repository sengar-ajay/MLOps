...
class InMemoryDatabaseLogger:
    ...
    def log_message(self, level: str, module: str, message: str, extra_data: Optional[Dict] = None):
        """
    ...
    def log_api_metric(self, endpoint: str, method: str, status_code: int, 
        ...
        with self.lock:
            ...
            ''', (endpoint, method, status_code, response_time, success, 
                  error_message, request_json, response_json))

    def log_model_metric(self, model_name: str, model_type: str, rmse: float, 
                        mae: float, r2_score: float, training_time: float, 
        ...
        with self.lock:
            ...
            self.connection.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (model_name, model_type, rmse, mae, r2_score, training_time, params_json))
    ...
    def get_api_metrics(self, endpoint: Optional[str] = None, limit: int = 100) -> List[Dict]:
        """
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            cursor = self.connection.execute('''
                ...
                GROUP BY level
            ''')
            stats['logs_by_level'] = {row['level']: row['count'] for row in cursor.fetchall()}
            ...
            # Model metrics count
            cursor = self.connection.execute('SELECT COUNT(*) as count FROM model_metrics')
