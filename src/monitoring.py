...
class APIMonitor:
    ...
    def __init__(self, api_url='http://localhost:5000', log_file='logs/api_monitor.log'):
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"Health check passed - Response time: {response_time:.3f}s")
            else:
    ...
    def test_prediction(self, sample_data: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(f"Prediction test passed - Response time: {response_time:.3f}s")
            else:
                self.logger.warning(f"Prediction test failed - Status: {response.status_code}")
    ...
    def run_monitoring_cycle(self, duration_minutes: int = 10, interval_seconds: int = 30):
        """
    ...
    def generate_report(self, output_dir: str = 'reports'):
        ...
        if 'response_time' in df.columns:
            ...
            plt.tight_layout()
            plt.savefig(f'{output_dir}/api_monitoring_report.png', dpi=300, bbox_inches='tight')
            plt.close()
            
            self.logger.info(f"Monitoring report saved to {output_dir}/api_monitoring_report.png")
        
        # Generate summary statistics
        summary = {
            'total_requests': len(df),
            'average_response_time': df['response_time'].mean() if 'response_time' in df.columns else None,
            'max_response_time': df['response_time'].max() if 'response_time' in df.columns else None,
            'min_response_time': df['response_time'].min() if 'response_time' in df.columns else None,
            'success_rate': df['is_healthy'].mean() if 'is_healthy' in df.columns else None,
            'monitoring_period': {
        ...
        self.logger.info(f"Summary statistics saved to {output_dir}/monitoring_summary.json")
...
def main():
    ...
    print("Basic monitoring completed. Check logs/ and reports/ directories for detailed results.")
