...
class DataDriftDetector:
    ...
    def detect_drift(self, new_data: pd.DataFrame, threshold: float = 0.05) -> Dict:
        ...
        try:
            ...
            drift_results["overall_drift"] = len(drift_results["drifted_features"]) > drift_threshold
            
            logger.info(f"Drift detection completed. Drifted features: {len(drift_results['drifted_features'])}")
        ...
        try:
            ...
            if os.path.exists(metrics_path):
                with open(metrics_path, 'r') as f:
                    metrics = json.load(f)
                logger.info(f"Baseline metrics loaded: RMSE={metrics.get('rmse', 'N/A')}")
                return metrics
        except Exception as e:
            logger.error(f"Error loading baseline metrics: {e}")
        return None
    
    def evaluate_current_performance(self, predictions: List[float], actuals: List[float]) -> Dict:
        """
        ...
        try:
            ...
            if self.baseline_metrics:
                rmse_degradation = (rmse - self.baseline_metrics["rmse"]) / self.baseline_metrics["rmse"]
                r2_degradation = (self.baseline_metrics["r2"] - r2) / self.baseline_metrics["r2"]
                
                performance_metrics.update({
                    "rmse_degradation": float(rmse_degradation),
                    "r2_degradation": float(r2_degradation),
                    "performance_degraded": rmse_degradation > self.performance_threshold or r2_degradation > self.performance_threshold
                })
...
class RetrainingTrigger:
    ...
    def should_retrain(self, new_data: pd.DataFrame, predictions: List[float] = None, 
                      actuals: List[float] = None) -> Dict:
        """
        ...
        try:
            ...
            if predictions and actuals and len(predictions) == len(actuals):
                performance_results = self.performance_monitor.evaluate_current_performance(predictions, actuals)
                retraining_decision["performance_analysis"] = performance_results
                
                if performance_results.get("performance_degraded", False):
                    retraining_decision["reasons"].append("Performance degradation detected")
                    retraining_decision["should_retrain"] = True
            ...
            data_volume_check = {
                ...
                "sufficient_data": len(new_data) >= self.config["min_samples_for_retraining"]
            }
            retraining_decision["data_volume_check"] = data_volume_check
            
            if not data_volume_check["sufficient_data"]:
                retraining_decision["reasons"].append("Insufficient new data for retraining")
            ...
            try:
                if os.path.exists("models/best_model_metrics.json"):
                    with open("models/best_model_metrics.json", 'r') as f:
                        metrics = json.load(f)
                        last_training = datetime.fromisoformat(metrics.get("training_timestamp", datetime.now().isoformat()))
                        days_since_training = (datetime.now() - last_training).days
                        
                        if days_since_training > self.config["max_days_since_last_training"]:
                            retraining_decision["reasons"].append(f"Model is {days_since_training} days old")
                            retraining_decision["should_retrain"] = True
            except Exception as e:
                logger.warning(f"Could not check last training time: {e}")
            
            # Final decision
            if retraining_decision["should_retrain"] and not self.config["enable_automatic_retraining"]:
                retraining_decision["should_retrain"] = False
                retraining_decision["reasons"].append("Automatic retraining is disabled")
            ...
            logger.info(f"Retraining decision: {retraining_decision['should_retrain']}, Reasons: {retraining_decision['reasons']}")
