...
import joblib
import logging
from data_preprocessing import load_processed_data, main as preprocess_main
...
def train_linear_regression(X_train, y_train, X_test, y_test):
    ...
    with mlflow.start_run(run_name="Linear_Regression"):
        ...
        logger.info(f"Linear Regression - RMSE: {metrics['rmse']:.4f}, MAE: {metrics['mae']:.4f}, R2: {metrics['r2']:.4f}")
        
        return model, metrics

def train_random_forest(X_train, y_train, X_test, y_test, n_estimators=100, max_depth=10, random_state=42):
    """
    ...
    with mlflow.start_run(run_name="Random_Forest"):
        ...
        logger.info(f"Random Forest - RMSE: {metrics['rmse']:.4f}, MAE: {metrics['mae']:.4f}, R2: {metrics['r2']:.4f}")
        
        return model, metrics

def train_gradient_boosting(X_train, y_train, X_test, y_test, n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42):
    """
    ...
    with mlflow.start_run(run_name="Gradient_Boosting"):
        ...
        logger.info(f"Gradient Boosting - RMSE: {metrics['rmse']:.4f}, MAE: {metrics['mae']:.4f}, R2: {metrics['r2']:.4f}")
...
def save_best_model(models_results, models_dir='models'):
    ...
    logger.info(f"Best model metrics: RMSE: {best_metrics['rmse']:.4f}, MAE: {best_metrics['mae']:.4f}, R2: {best_metrics['r2']:.4f}")
