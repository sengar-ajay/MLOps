...
def test_preprocess_data():
    ...
    # Preprocess data
    X_train, X_test, y_train, y_test, scaler = preprocess_data(X, y, test_size=0.3, random_state=42)
    ...
    assert all(abs(mean) < 0.1 for mean in train_means), f"Means not close to 0: {train_means}"
    assert all(abs(std - 1.0) < 0.1 for std in train_stds), f"Stds not close to 1: {train_stds}"
...
def test_preprocess_data_different_test_size():
    ...
    test_size = 0.1
    X_train, X_test, y_train, y_test, scaler = preprocess_data(X, y, test_size=test_size)
...
def test_save_and_load_processed_data(temp_data_dir):
    ...
    expected_files = ['X_train.csv', 'X_test.csv', 'y_train.csv', 'y_test.csv', 'scaler.pkl']
    for file in expected_files:
        assert os.path.exists(os.path.join(temp_data_dir, file))
    
    # Load processed data
    X_train_loaded, X_test_loaded, y_train_loaded, y_test_loaded, scaler_loaded = load_processed_data(temp_data_dir)
    ...
    pd.testing.assert_series_equal(y_train.reset_index(drop=True), y_train_loaded.reset_index(drop=True))
    pd.testing.assert_series_equal(y_test.reset_index(drop=True), y_test_loaded.reset_index(drop=True))
...
def test_reproducibility():
    ...
    X_train1, X_test1, y_train1, y_test1, scaler1 = preprocess_data(X, y, random_state=42)
    X_train2, X_test2, y_train2, y_test2, scaler2 = preprocess_data(X, y, random_state=42)
...
def test_different_random_states():
    ...
    X_train1, X_test1, y_train1, y_test1, scaler1 = preprocess_data(X, y, random_state=42)
    X_train2, X_test2, y_train2, y_test2, scaler2 = preprocess_data(X, y, random_state=123)
...
def test_scaler_transform_consistency():
    ...
    X_train_manual = pd.DataFrame(
        scaler.transform(scaler.inverse_transform(X_train)),
